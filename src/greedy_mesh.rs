...
//! # 适用场景
//!
//! 此算法当前仅用于水方块（BlockId = 5），因为：
//! - 水方块通常需要半透明渲染，合并后可以减少 overdraw
//! - 水方块材质简单，UV 拉伸效果可接受
//! - 其他不透明方块使用标准渲染即可
//!
//! # 性能预期
...
use crate::chunk::{BlockId, CHUNK_SIZE, ChunkData, ChunkNeighbors, should_cull_face};
    ...
    F: Fn(u8, &str) -> (f32, f32, f32, f32),
{
    ...
    for i in 0..4 {
        ...
        let local_u = verts[i][face.u_axis] - u_start_f;
        let local_v = verts[i][face.v_axis] - v_start_f;
        face_uvs[i][0] = layer_index + local_u;
        face_uvs[i][1] = local_v;
    }
